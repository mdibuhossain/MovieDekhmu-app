...
export const addMovie = async (payload) => {
  return addDoc(moviesCollection, payload)
    .then((docRef) => {
...
export const updateMovie = async (payload) => {
  const movieRef = doc(db, "movies", payload.id);
...
export const getMovies = async (filters = {}) => {
  const movies = [];
  let moviesQuery = moviesCollection;
  ...
  if (filters.releaseYear) {
    moviesQuery = query(moviesQuery, where("filmType", "==", filters.filmType));
  }
  if (filters.releaseYear) {
